...
impl ResourceOrigin {
    ...
    pub(crate) async fn read(
        ...
        if !AppToolPolicyEvaluator::new(&binding.config().config_layer_stack).policy_enabled(
            /*connector_id*/ Some(&self.connector_id),
            tool_info.tool.name.as_ref(),
...
pub(crate) fn apps_config_from_layer_stack(
...
impl<'a> AppToolPolicyEvaluator<'a> {
    ...
    pub(crate) fn policy_enabled(
        ...
        let Some(apps_config) = self.apps_config.as_ref() else {
            // No apps configuration: tools are enabled by default.
            return true;
        };
        ...
        let destructive_hint = destructive_hint.unwrap_or(true);
        let open_world_hint = open_world_hint.unwrap_or(true);
