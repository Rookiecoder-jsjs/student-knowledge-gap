"""gateway school-authz 身份签发测试（§6.3 / §5.5 首选路）。

覆盖：
- `_sign_school_token`：格式与 sc 后端 auth.py 一致（`teacher_id.exp.sig`，HMAC-SHA256 hex）；
- `_teacher_identity_env`：SC_AUTH_SECRET 配置 → 只带签名 token（不再注入裸
  SC_MCP_TEACHER_ID）；未配置 → 维持旧兜底裸 SC_MCP_TEACHER_ID（灰度安全）。
"""
