...
- 已补任务积压、最老等待时间、失败任务数、备份验证时间和磁盘水位；延迟直方图仍待补；
- gateway 尚未暴露独立 Prometheus 指标，心跳仍是摘要上报；
- 告警模板已提供于 deploy/monitoring；集中日志接入仍需学校或运营平台配置；
- SQLite 备份现在自动执行隔离恢复校验；跨服务故障演练仍由运维按手册执行。
...
3. 已补任务、备份、磁盘指标；网关桥接指标和延迟直方图仍待补；
4. 待提供可选的 Prometheus scrape 配置，不把 Prometheus 作为默认运行服务。

### 批次 C：告警与演练（模板与手册已落地，运行环境需接入）
